...
var DatePicker = new Class({
	...
	initialize: function(dp){
		...
		// Options defaults
		this.dayChars = 3; // number of characters in day names abbreviation
		//this.dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
		this.dayNames = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
		this.daysInMonth = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
		this.format = 'yyyy-mm-dd';
		//this.monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
		this.monthNames = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'Nopember', 'Desember'];
	...
	create: function(dp){
		...
		else if (dp.options.yearOrder == 'both'){
			for (var y = (dp.options.yearStart + dp.options.yearRange + 1); y > dp.options.yearStart; y--){
				yearSel.options[i] = new Option(y, y);
				i++;
			}
			for (var y = dp.options.yearStart; y > (dp.options.yearStart - dp.options.yearRange - 1); y--){
